...
{
    public class NewContractTypeViewModel : JedenViewModel<NewContractTypeCreateEditVM>
    {
        private Window _window;
        #region Konstruktor
        public NewContractTypeViewModel(Window window) : base("ContractType")
        {
            item = new ContractTypeCreateEditVM();
            _window = window;
        }
        public override async void Save()
        {
            await RequestHelper.SendRequestAsync(URLs.CONTRACTTYPE, HttpMethod.Post, item, null);
            Messenger.Default.Send("ContractTypeRefresh");
            _window.Close();
        }
        #endregion
