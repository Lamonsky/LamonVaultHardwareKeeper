...
namespace AdministrationApp.ViewModels.EditViewModel
{
    class EditPrinterViewModel : JedenViewModel<PrintersCreateEditVM>
    {
        ...
        {
            await RequestHelper.SendRequestAsync(URLs.PRINTER_ID.Replace("{id}", item.Id.ToString()), HttpMethod.Put, item, null);
        ...
        public async void setForeignKeys()
        {
            ...
            PrinterTypeVM ctypevm = await RequestHelper.SendRequestAsync<object, PrinterTypeVM>(URLs.PRINTERTYPE_ID.Replace("{id}", item.PrinterType.ToString()), HttpMethod.Get, null, null);
            PrinterModelVM cmodelvm = await RequestHelper.SendRequestAsync<object, PrinterModelVM>(URLs.PRINTERMODEL_ID.Replace("{id}", item.Model.ToString()), HttpMethod.Get, null, null);
