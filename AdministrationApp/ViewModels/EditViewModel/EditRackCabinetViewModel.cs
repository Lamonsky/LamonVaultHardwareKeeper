...
namespace AdministrationApp.ViewModels.EditViewModel
{
    class EditRackCabinetViewModel : JedenViewModel<RackCabinetCreateEditVM>
    {
        ...
        {
            await RequestHelper.SendRequestAsync(URLs.RACKCABINET_ID.Replace("{id}", item.Id.ToString()), HttpMethod.Put, item, null);
        ...
        public async void setForeignKeys()
        {
            ...
            RackCabinetTypeVM ctypevm = await RequestHelper.SendRequestAsync<object, RackCabinetTypeVM>(URLs.RACKCABINETTYPE_ID.Replace("{id}", item.CabinetType.ToString()), HttpMethod.Get, null, null);
            RackCabinetModelVM cmodelvm = await RequestHelper.SendRequestAsync<object, RackCabinetModelVM>(URLs.COMPUTERMODEL_ID.Replace("{id}", item.Model.ToString()), HttpMethod.Get, null, null);
