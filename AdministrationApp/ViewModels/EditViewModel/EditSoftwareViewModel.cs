...
namespace AdministrationApp.ViewModels.EditViewModel
{
    class EditSoftwareViewModel : JedenViewModel<SoftwareCreateEditVM>
    {
        ...
        {
            await RequestHelper.SendRequestAsync(URLs.SOFTWARE_ID.Replace("{id}", item.Id.ToString()), HttpMethod.Put, item, null);
        ...
        {
            StatusVM statusvm = await RequestHelper.SendRequestAsync<object, StatusVM>(URLs.STATUS_ID.Replace("{id}", item.Status.ToString()), HttpMethod.Get, null, null);
            ...
            ManufacturerVM manufacturerVM = await RequestHelper.SendRequestAsync<object, ManufacturerVM>(URLs.MANUFACTURER_ID.Replace("{id}", item.Publisher.ToString()), HttpMethod.Get, null, null);
