...
{
    public class EditKnowledgebaseCategoryViewModel : JedenViewModel<KnowledgeBaseCategoryCreateEditVM>
    {
        ...
        #region Konstruktor
        public EditKnowledgebaseCategoryViewModel(Window window, KnowledgeBaseCategoryCreateEditVM vm) : base("KnowledgebaseCategory")
        {
            item = vm;
            _window = window;
            setForeignKeys();
        ...
        {
            await RequestHelper.SendRequestAsync(URLs.KNOWLEDGEBASECATEGORY_ID.Replace("{id}", item.Id.ToString()), HttpMethod.Put, item, null);
            Messenger.Default.Send("KnowledgebaseCategoryRefresh");
            _window.Close();
        }
        #endregion
