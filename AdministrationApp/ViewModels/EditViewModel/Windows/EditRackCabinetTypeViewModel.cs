...
namespace AdministrationApp.ViewModels.EditViewModel.Windows
{
    public class EditRackCabinetTypeViewModel : JedenViewModel<RackCabinetTypeCreateEditVM>
    {
        ...
        #region Konstruktor
        public EditRackCabinetTypeViewModel(Window window, RackCabinetTypeCreateEditVM vm) : base("RackCabinetType")
        {
            item = vm;
            _window = window;
            setForeignKeys();
        ...
        {
            await RequestHelper.SendRequestAsync(URLs.RACKCABINETTYPE_ID.Replace("{id}", item.Id.ToString()), HttpMethod.Put, item, null);
