...
namespace AdministrationApp.ViewModels.AllViewModel
{
    class AllDeviceModelViewModelWindow : WszystkieViewModel<DeviceModelVM>
    {
        ...
        {
            List = await RequestHelper.SendRequestAsync<object, List<DeviceModelVM>>(URLs.DEVICEMODEL, HttpMethod.Get, null, null);
        }

        public async override void Remove()
        {
            await RequestHelper.SendRequestAsync(URLs.DEVICEMODEL_ID.Replace("{id}", ChosenItem.Id.ToString()), HttpMethod.Delete, ChosenItem, null);
            load();
        }
